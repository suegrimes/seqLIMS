...
class Item < ActiveRecord::Base
  ...
  def self.find_all_unique(condition_array=nil)
    self.where(*condition_array).group("item_description, catalog_nr").all
    #self.find(:all, :group => "item_description, catalog_nr",
    #               :conditions => condition_array)
  end
  
  def self.find_all_by_date(condition_array=nil)
    self.includes(:order).where(*condition_array).order("DATE(items.created_at) DESC, orders.po_number").all
